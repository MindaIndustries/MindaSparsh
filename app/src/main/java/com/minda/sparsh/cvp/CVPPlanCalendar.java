...
public class CVPPlanCalendar extends AppCompatActivity implements OnDateSelectedListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        initWeekSpinner();
        String currentDate = Calendar.getInstance().get(Calendar.YEAR)+"-"+(Calendar.getInstance().get(Calendar.MONTH)+1)+"-"+Calendar.getInstance().get(Calendar.DAY_OF_MONTH);
        getWeek(currentDate);
        initCustomerSpinner();
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                saveCalendarBooking(Integer.parseInt(weekdayId),customerId,custLocId,Integer.parseInt(meetingTypeId),"2022",empcode, Integer.parseInt(calendartypeId));
            }
        });
    }
    @Override
                    ...
                    if(locationModel!=null){
                        ...
                        if(list!=null && list.size()>0){
                            ...
                            for (LocationModel.LocationData locationData : list) {
                                locationList.add(locationData.getLocation());
                            }
            ...
            public void onTaskComplte(CarotResponse carotResponse) {
                if(carotResponse.getStatuscode() ==HttpsURLConnection.HTTP_OK){
                    ...
                    if(saveCalendarResponse!=null){
                        ...
                        if(data!=null){
                            if(data.equalsIgnoreCase("Data is saved successfully")){
                                Toast.makeText(CVPPlanCalendar.this,"Meeting Booked successfully",Toast.LENGTH_LONG).show();
                                onBackPressed();
                            }
                        }
                    }
